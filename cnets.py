...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        # === FSD: Prepare logits for initial top-k proposals ===
        if top_k > 0:
            node_logits_chunks.append(last_headout.repeat_interleave(top_k, dim=0))
        ...
        if self.config.vocab_size == self.config.draft_vocab_size:
            ...
            # Clamp topk_index to draft_vocab_size to prevent index out of bounds
            safe_topk_index = torch.clamp(topk_index, max=self.config.draft_vocab_size - 1)
            ss_token.append(safe_topk_index + self.d2t[safe_topk_index])
            input_ids = safe_topk_index + self.d2t[safe_topk_index]
        ...
        for i in range(depth):
            ...
            # === FSD: Save logits for each branch in the speculative tree ===
            if top_k > 0:
                node_logits_chunks.append(last_headout.repeat_interleave(top_k, dim=0))
            ...
            if self.config.vocab_size == self.config.draft_vocab_size:
                ...
                # Clamp topk_index to draft_vocab_size to prevent index out of bounds
                # This happens when vocab_size > draft_vocab_size (e.g., Qwen3 151936 vs draft 32000)
                safe_topk_index = torch.clamp(topk_index, max=self.config.draft_vocab_size - 1)
                safe_input_ids = torch.clamp(input_ids, max=self.config.draft_vocab_size - 1)
                input_ids = safe_input_ids + self.d2t[safe_input_ids]
                ss_token.append(safe_topk_index + self.d2t[safe_topk_index])
        ...
        # Ensure total_tokens doesn't exceed the available scores
        actual_total_tokens = min(total_tokens, scores_list.size(0))
        if actual_total_tokens < total_tokens:
            print(
                f"Warning: Requested {total_tokens} tokens but only {scores_list.size(0)} available. Using {actual_total_tokens}."
            )
        ...
        leaf_num = actual_total_tokens - noleaf_num
        ...
        # === FSD: Organize draft_logits to match draft_tokens ===
        # node_logits_chunks aligns with flattened scores_list order
        if node_logits_chunks:
            logits_stack = torch.cat(node_logits_chunks, dim=0)
            selected_logits = logits_stack[top_scores_index]
            draft_logits = selected_logits[None]
        else:
            draft_logits = None

        return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, draft_logits, draft_logits
